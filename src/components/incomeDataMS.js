...
const IncomeDataMS = () => {
  ...
  useEffect(() => {
    axios
      .get("/api/data/mississippi_income_data")
      .then((response) => {
        const dataRetrieved = response.data;
        setData(dataRetrieved.data);
      })
      .catch((error) => {
        console.error("Error fetching Mississippi income data:", error);
      });
  }, []);

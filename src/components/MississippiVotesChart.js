...
const MississippiVotesChart = () => {
  const chartRef = useRef(null);

  const data = [
    { party: "Democratic", votes: 1234800, color: "#4A90D9", gradientId: "demGradMS" },
    { party: "Republican", votes: 1293600, color: "#E85C50", gradientId: "repGradMS" },
  ];
  ...
  useEffect(() => {
    ...
    g.selectAll(".bar")
      ...
      .attr("d", d => {
        const bx = x(d.party);
        const by = height;
        return `M${bx},${by} L${bx+barW},${by} L${bx+barW},${by} L${bx},${by} Z`;
      })
...
export default MississippiVotesChart;

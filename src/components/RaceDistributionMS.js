...
const RaceDistributionMS = () => {
  const chartRef = useRef(null);
  const [data, setData] = useState(null);

  useEffect(() => {
    axios.get('http://localhost:8080/api/data/mississippi_race_data')
      .then((response) => {
        const dataRetrieved = response.data;
        setData(dataRetrieved);
        console.log("MS race data: ", dataRetrieved);
      })
      .catch((error) => {
  ...
  useEffect(() => {
    ...
    const margin = { top: 25, right: 15, bottom: 20, left: 50 };
    ...
    svg
      ...
      .text('Race Distribution in Mississippi (%)');
...
export default RaceDistributionMS;
